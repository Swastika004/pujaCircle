...
export const AdminPriestDetailsPage: React.FC = () => {
  ...
  if (!priest) {
    return (
      <div className="container max-w-3xl py-12 space-y-6 text-stone-900">
        ...
        <div className="text-center p-10 space-y-4 rounded-xl border-2 border-amber-300 bg-white shadow-sm">
          <div className="w-14 h-14 rounded-md bg-amber-100 flex items-center justify-center mx-auto text-amber-700 border border-amber-300">
            <AlertTriangle className="w-7 h-7" />
          </div>
          <div className="space-y-1">
            <h2 className="text-lg font-bold font-serif text-stone-900">
              Priest Record Not Found
            </h2>
          ...
          <Link to="/admin/priests">
            <Button
              size="sm"
              className="text-xs mt-2 bg-[#780016] hover:bg-red-800 text-white font-bold rounded-md border border-amber-400 h-10 px-5"
            >
    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        {approvalStatus === "PENDING" && (
          <Badge
            variant="outline"
            className="border-amber-400 text-amber-900 bg-amber-100 text-xs px-3 py-1 gap-1 font-bold"
          >
        ...
        {approvalStatus === "REJECTED" && (
          <Badge
            variant="destructive"
            className="text-xs px-3 py-1 gap-1 font-bold"
          >
  ...
  return (
    <div className="container max-w-4xl py-8 space-y-6 pb-16 text-stone-900">
      ...
      <div className="border-2 border-amber-300 bg-white shadow-sm rounded-xl overflow-hidden p-6 sm:p-7">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            {priest.profileImageUrl ? (
              <img
                ...
                alt={priest.fullName}
                className="w-18 h-18 rounded-md object-cover border-2 border-amber-400 shadow-sm"
              />
            ) : (
              <div className="w-18 h-18 rounded-md bg-amber-100 text-amber-900 border-2 border-amber-400 flex items-center justify-center font-bold text-2xl font-serif">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-stone-500 font-mono">
                Scholar ID: {priest.id} • Registered:{" "}
                {new Date(priest.createdAt).toLocaleDateString("en-IN", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })}
          ...
          <div className="sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0 w-full sm:w-auto flex sm:flex-col justify-between items-center sm:items-end gap-1 text-xs">
            {priest.rating && (
              <div className="flex items-center gap-1 font-bold text-amber-900 bg-amber-100 px-2.5 py-1 rounded-sm border border-amber-300">
      ...
      {(priest.accountStatus === "BANNED" ||
        ...
        <div
          className={`p-5 rounded-md border-2 flex items-start gap-3.5 ${priest.accountStatus === "BANNED" ? "border-red-300 bg-red-50/70" : "border-amber-300 bg-amber-50/70"}`}
        >
          <AlertTriangle
            className={`w-5 h-5 shrink-0 mt-0.5 ${priest.accountStatus === "BANNED" ? "text-red-700" : "text-amber-700"}`}
          />
          <div className="space-y-1 text-xs">
            <p className="font-bold text-stone-900">
              Administrative Status ({priest.approvalStatus} /{" "}
              {priest.accountStatus})
            </p>
            <p className="text-stone-700">
              {priest.banReason ||
                priest.rejectionReason ||
                (priest.approvalStatus === "PENDING"
                  ? "Priest registration is currently awaiting verification council signoff."
      ...
      <div className="flex flex-col md:flex-row gap-6 w-full">
        ...
        <div className="w-full md:w-1/2 border-2 border-amber-300 bg-white shadow-sm rounded-xl p-6 space-y-4">
      ...
      <div className="border-2 border-amber-300 bg-white shadow-sm rounded-xl p-6 sm:p-7 space-y-3">
        <div>
          <h3 className="text-base font-serif font-bold text-stone-950">
            Administrative Actions & Lifecycle Control
          </h3>
          <p className="text-xs text-stone-600">
            Manage application verification state, handle disciplinary
            suspensions, or permanently remove profile.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-stone-200">
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {priest.approvalStatus === "PENDING" && (
              <>
                <Button
                  ...
                  className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-10 px-5 rounded-md gap-1.5 shadow-xs cursor-pointer puja-btn-tap"
                >
                ...
                <Button
                  ...
                  className="text-xs text-red-700 hover:bg-red-50 border-red-200 font-bold h-10 px-4 rounded-md gap-1.5 cursor-pointer"
                >
            ...
            {priest.accountStatus === "ACTIVE" && (
              <Button
                ...
                className="text-xs h-10 px-4 rounded-md font-bold gap-1.5 cursor-pointer"
              >
            ...
            {priest.accountStatus === "BANNED" && (
              <Button
                ...
                className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-10 px-5 rounded-md gap-1.5 cursor-pointer"
              >
            ...
            {priest.approvalStatus === "REJECTED" && (
              <Button
                ...
                className="text-xs gap-1.5 h-10 px-4 rounded-md font-bold border-stone-300"
              >
          ...
          <Button
            ...
            className="text-xs text-red-700 hover:bg-red-50 gap-1.5 h-10 px-3 rounded-md font-bold cursor-pointer"
          >
        ...
        <DialogContent className="sm:max-w-md p-6 rounded-lg bg-white border-2 border-amber-300 shadow-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-stone-600">
              Provide a clear administrative reason for rejecting{" "}
              {priest.fullName}'s application.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-2 py-2">
            ...
            <Textarea
              ...
              className="text-xs min-h-24 rounded-md border-stone-300 focus:ring-amber-500 bg-white"
            />
          </div>
          ...
          <DialogFooter className="gap-2 sm:gap-0 flex flex-col-reverse sm:flex-row">
            <Button
              ...
              className="text-xs rounded-md h-10 px-4"
            >
              Cancel
            </Button>
            <Button
              ...
              className="text-xs rounded-md h-10 px-5 font-bold"
            >
              Confirm Rejection
            </Button>
          </DialogFooter>
      ...
      <Dialog open={isBanDialogOpen} onOpenChange={setIsBanDialogOpen}>
        <DialogContent className="sm:max-w-md p-6 rounded-lg bg-white border-2 border-amber-300 shadow-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-stone-600">
              Revoke platform access for {priest.fullName}. Please state the
              infraction or reason.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent className="p-6 rounded-lg bg-white border-2 border-amber-300 shadow-xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs text-stone-600">
              ...
              from the platform roster? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex flex-col-reverse sm:flex-row gap-2">
            <AlertDialogCancel className="text-xs rounded-md h-10 px-4">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleConfirmDelete}
              className="bg-red-700 text-white hover:bg-red-800 text-xs rounded-md h-10 px-5 font-bold"
            >
              Delete Record
            </AlertDialogAction>
          </AlertDialogFooter>
