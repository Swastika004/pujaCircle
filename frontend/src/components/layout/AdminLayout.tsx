...
/**
 * AdminLayout
 * Royal Sanctum Administrative Console for Operations Staff (ADMIN role).
 * 100% Flexbox, Royal Sanctum Maroon (#240306) fixed sidebar and header,
 * Haldi gold trims, zero gradients, zero CSS grids.
 */
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-background relative">
      <SacredBackground />
      {/* Admin Sidebar - Hidden on mobile, fixed on md+ screens */}
      ...
      {/* Main Workspace Shifted for fixed sidebar */}
        ...
        <header className="sticky top-0 z-20 h-16 border-b border-amber-300/90 bg-white/95 backdrop-blur-xs text-stone-900 flex items-center justify-between px-4 sm:px-6 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <ShieldAlert className="h-3 w-3 text-amber-600" /> Platform Authority
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-stone-600 font-mono hidden sm:inline font-medium">
              {user?.email || "admin@pujacircle.demo"}
            </span>
            <Button
              variant="ghost"
              size="sm"
